...
import { Bot, User, Clock, Plus, Download, FileText, XCircle, Upload, RefreshCcw, Loader2, Wifi, WifiOff, Server, Coffee, Zap } from 'lucide-react';
...
export default function InterviewPage() {
  ...
  useEffect(() => {
    ...
    const checkServer = async () => {
        try {
            // We ping the root "/" which returns 404 on FastAPI if no route, but means "Alive"
            // Or we can ping a non-existent endpoint, getting a response is enough.
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 5000); // 5s timeout per ping
  ...
  // --- 3. BATCH LISTENING LOGIC (NO CHANGES) ---
  useEffect(() => {
  ...
  return (
    <div className="h-screen bg-gray-950 text-gray-100 font-sans flex flex-col overflow-hidden">
      ...
      {step === 1 && (
        <div className="max-w-xl mx-auto mt-6 space-y-6 bg-gray-900 p-8 rounded-2xl border border-gray-800 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
           <div className="text-center mb-6">
                <FileText size={48} className="mx-auto text-blue-500 mb-4"/>
                <h2 className="text-2xl font-bold">Setup Interview</h2>
           </div>

           {/* --- INNOVATIVE SERVER STATUS CARD --- */}
           {serverStatus !== 'ready' && (
               <div className="bg-amber-900/20 border border-amber-800/50 p-4 rounded-xl flex flex-col gap-3 animate-pulse">
                   <div className="flex items-caenter gap-3 text-amber-300">
                       <Server className="animate-bounce" size={24} />
                       <div>
                           <h3 className="font-bold text-sm">Waking up the AI Brains... 😴</h3>
                           <p className="text-xs opacity-80">This usually takes ~40-50 seconds (Free Tier)</p>
                       </div>
                   </div>
                   ...
                   <div className="bg-gray-900/50 p-3 rounded-lg flex items-start gap-2 border border-amber-500/10">
                       <Coffee className="text-amber-400 shrink-0" size={16} />
                       <div className="text-xs text-gray-400">
                           <strong className="text-amber-200">P.S. Hate the wait?</strong> We rely on free hosting. 
                           <span className="block mt-1">If you like this project, consider <a href="#" className="text-blue-400 underline">donating a coffee</a> so we can upgrade to lightning-fast GPU servers! 🚀⚡</span>
                       </div>
                   </div>
               </div>
           )}
                    ...
                    <div className="h-16 flex justify-center items-center w-full flex-none">
                        {status === 'Listening' && (
                            <div className="flex items-center gap-4 bg-gray-800/80 px-6 py-2 rounded-full border border-gray-700 backdrop-blur-sm animate-in fade-in slide-in-from-top-4">
                                ...
                                <button onClick={addTime} className="hover:bg-gray-700 rounded-full text-blue-400 p-1"><Plus size={20}/></button>
                            </div>
                        )}
                    </div>
